...
Params: `src` (media URL; add `type=video` for extension-less video URLs), `aspect`, `grid`, `shape`, `size`, `threshold`, `dot` / `bg` (hex, `#` optional), `brightness`, `contrast`, `gamma`, `dither`, `multicolor`, `invert`, `motion`, `amount`, `speed`, `phase`. New exports set `aspect=source`; old embed URLs without it retain their square frame. Point `src` at any CORS-accessible clip, or commit your clip under `webgl/assets/` in this repo — GitHub Pages serves it with the right headers.
...
- Invert toggle in the live component to swap foreground and background
